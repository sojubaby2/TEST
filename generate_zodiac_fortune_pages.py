...
SIGNS = [
    {"id": "aries", "emoji": "🐏", "name": "양자리"},
    {"id": "taurus", "emoji": "🐂", "name": "황소자리"},
    {"id": "gemini", "emoji": "👯", "name": "쌍둥이자리"},
    {"id": "cancer", "emoji": "🦀", "name": "게자리"},
    {"id": "leo", "emoji": "🦁", "name": "사자자리"},
    {"id": "virgo", "emoji": "🌾", "name": "처녀자리"},
    {"id": "libra", "emoji": "⚖️", "name": "천칭자리"},
    {"id": "scorpio", "emoji": "🦂", "name": "전갈자리"},
    {"id": "sagittarius", "emoji": "🏹", "name": "사수자리"},
    {"id": "capricorn", "emoji": "🐐", "name": "염소자리"},
    {"id": "aquarius", "emoji": "🏺", "name": "물병자리"},
    {"id": "pisces", "emoji": "🐟", "name": "물고기자리"},
]

COLORS = {
    "aries": "#EF4444", "taurus": "#16A34A", "gemini": "#EAB308", "cancer": "#38BDF8",
    "leo": "#F97316", "virgo": "#65A30D", "libra": "#F472B6", "scorpio": "#7C3AED",
    "sagittarius": "#DC2626", "capricorn": "#78716C", "aquarius": "#06B6D4", "pisces": "#6366F1",
}
...
<title>오늘의 {name} 운세 | 테스트맛집</title>
<meta name="description" content="{name} 오늘의 총운, 애정운, 금전운, 건강운과 행운의 아이템을 확인해보세요." />

<meta property="og:type" content="website" />
<meta property="og:title" content="오늘의 {name} 운세 {emoji}" />
<meta property="og:description" content="오늘의 총운 · 애정운 · 금전운 · 건강운을 확인해보세요!" />
<meta property="og:image" content="../../assets/img/og/zodiac-fortune-{id}.png" />
...
<main class="wrap">
  ...
  <div class="card">
    <p style="margin:0;font-size:14px;color:var(--text-sub);">{name}는(은) <b style="color:var(--text-main);" id="traitLabel"></b>이 돋보이는 별자리예요.</p>
  ...
  <p class="section-title" style="margin-top:28px;">나와 잘 맞는 별자리는?</p>
  ...
  <a class="btn btn-secondary" href="index.html" style="margin-top:20px;">🔄 다른 별자리 보기</a>
...
<script src="../../assets/js/zodiac-fortune-data.js"></script>
<script src="../../assets/js/main.js"></script>
<script>
  (function () {{
    var signId = "{id}";
    var z = getZodiacSignById(signId);
    var fortune = getTodayZodiacFortune(signId);

    document.getElementById("traitLabel").textContent = z.trait;
    ...
    renderCompatSection("compatSection", ZODIAC_SIGN_LIST, z.compat);

    document.getElementById("shareBtn").addEventListener("click", function () {{
      shareCurrentPage(
        "오늘의 " + z.title + " 운세 " + z.emoji,
        "내 오늘의 별자리 운세는? 테스트맛집에서 확인해봐!"
      );
    }});
...
for s in SIGNS:
    html = TEMPLATE.format(
        name=s["name"], emoji=s["emoji"], id=s["id"], color=COLORS[s["id"]]
    )
    with open(f"tests/zodiac-fortune/result-{s['id']}.html", "w", encoding="utf-8") as f:
        f.write(html)
    print("wrote", s["id"])

print("done,", len(SIGNS), "files")
